...
void DomRadioMasterAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    const float initDecay = decayParam ? decayParam->load() : 0.0f;
    const float initAge = ageParam ? ageParam->load() : 0.0f;
    const float initMixAmount = mixParam ? mixParam->load() / 100.0f : 1.0f;

    wetChainL.eq.updateParameters(initSpeedNorm, initEqStd, initAirGain, initDecay, initAge, initTapeProfile, initMixAmount);
    wetChainR.eq.updateParameters(initSpeedNorm, initEqStd, initAirGain, initDecay, initAge, initTapeProfile, initMixAmount);
...
void DomRadioMasterAudioProcessor::resetCacheVariables()
{
