...
SaturationIndicator::SaturationIndicator(DomRadioMasterAudioProcessor& p) : processor(p)
{
    startTimerHz(30);
}
...
void SaturationIndicator::timerCallback()
{
    ...
    slewSat = processor.getSlewSaturationLevel();
    repaint();
}
...
void SaturationIndicator::drawVintageLED(juce::Graphics& g, juce::Rectangle<float> bounds, float level, const juce::String& label)
{
    ...
    const juce::Rectangle<float> ledArea(bounds.getCentreX() - ledSize * 0.5f, bounds.getY() + 15.0f, ledSize, ledSize);
...
void SaturationIndicator::paint(juce::Graphics& g)
{
    ...
    drawVintageLED(g, juce::Rectangle<float>(0.0f, 0.0f, w, (float)getHeight()), inputSat, "PRE");
    drawVintageLED(g, juce::Rectangle<float>(w, 0.0f, w, (float)getHeight()), tapeSat, "TAPE");
    drawVintageLED(g, juce::Rectangle<float>(w * 2.0f, 0.0f, w, (float)getHeight()), slewSat, "SLEW");
}
