...
EQGraphLED::EQGraphLED(DomRadioTrackAudioProcessor& p) : processor(p)
{
    startTimerHz(30);
}
...
juce::Path EQGraphLED::buildResponsePath(const DomRadioTrackAudioProcessor::TapeDisplayState& state) const
{
    juce::Path path;
...
juce::Path EQGraphLED::buildNoiseFloorPath(const DomRadioTrackAudioProcessor::TapeDisplayState& state, float noise, float age) const
{
    juce::Path path;
    const int w = getWidth();
    const float h = (float)getHeight();
    
    float noiseHeight = 3.0f + (noise * 30.0f) + (age * 12.0f) + (state.effectiveTapeActivity * 18.0f);
    ...
    for (int i = 0; i <= w; i += 4)
    {
        ...
        float y = h - (noiseHeight * 0.4f) + jitter;
...
void EQGraphLED::paint(juce::Graphics& g)
{
    ...
    float satIntensity = juce::jlimit(0.0f, 1.0f, 
        state.effectivePreampActivity * 1.3f + state.effectiveTapeActivity * 1.5f);
    
    float detailActivity = processor.getDetailActivity();
    float totalIntensity = juce::jlimit(0.0f, 1.0f, satIntensity + detailActivity * 0.8f);

    float baseAlpha = 0.18f + totalIntensity * 0.25f;
    ...
    float glowThickness = 1.6f + totalIntensity * 2.5f;
...

    {
        ...
        juce::ColourGradient fg(phosphor.withAlpha(0.10f + totalIntensity * 0.08f), 0.0f, gainToY(maxDb),
    ...
    {
        auto noisePath = buildNoiseFloorPath(state, noiseLvl, ageLvl);
    ...
    juce::ColourGradient glass(juce::Colours::white.withAlpha(0.08f),
                               ...
                               b.getCentreX(), b.getY() + b.getHeight() * 0.35f, false);
